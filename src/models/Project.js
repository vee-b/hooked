...
export class Project {
  constructor({ id = uuidv4(), date_time = new Date(), image_path = 'No Image', is_sent = false, attempts = 0, grade = 'Unknown'}) {
    this.id = id;
    ...
    if (typeof date_time === 'string') {
      ...
      this.date_time = date_time; // If it's already a Date, use it directly
    }
  ...
  /**
   * @param {{ id: any; date_time: string | number | Date; image_path: any; is_sent: number; attempts: any; grade: any; }} map
   */
  static fromMap(map) {
    return new Project({
      id: map.id,
